...
```
```{r tune-lin-pre, echo=F, include=F, eval=T}
path_lin <- here::here('tune_lin.rds')
path_exists_lin <- file.exists(path_lin)

```
```{r tune-lin, eval=!path_exists_lin}
...
```
```{r tune-lin-post-export, echo=F, include=F, eval=!path_exists_lin}
write_rds(tune_lin, path_lin)

```
```{r tune-lin-post-import, echo=F, include=F, eval=T}
...
```
```{r imp-lin-2, echo=F, include=F, eval=F}
...
```
```{r tune-xg-prep, echo=F, include=F}
path_xg <- here::here('tune_xg.rds')
path_exists_xg <- file.exists(path_xg)

```
```{r tune-xg, echo=F, include=F, eval=!path_exists_xg}
tune_xg <-
  wf_xg %>% 
  tune_grid(
    folds,
...
```
```{r tune-xg-post-export, echo=F, include=F, eval=!path_exists_xg}
write_rds(tune_xg, path_xg)

```
```{r tune-xg-post-import, echo=F, include=F, eval=T}
